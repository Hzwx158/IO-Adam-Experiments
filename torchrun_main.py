...
@torch.no_grad()
def evaluate_model(model, preprocess_batched, pad_idx, global_rank, world_size, device, batch_size, ds):
    _time = time.time()
    if ds == "c4":
        val_data = datasets.load_dataset("allenai/c4", "en", split="validation", streaming=True, trust_remote_code=True) #DGX
    elif ds == "minipile":
        val_data = datasets.load_dataset("JeanKaddour/minipile", split="validation")
    else:
        raise NotImplementedError(f"dataset {ds}")
    ...
    for batch in val_data_mapped.batch(batch_size=batch_size):
        if isinstance(batch, list):
            ...
            for k in batch[0].keys():
                ...
                b[k] = torch.tensor(tmp).to(device)

            labels = b["input_ids"].clone()
            labels[labels == pad_idx] = -100
            loss = model(**b, labels=labels).loss
            total_loss += loss.detach()
            evaluated_on_tokens += (b["input_ids"] != pad_idx).sum().item() * world_size
...
def main(args):
    ...
    if global_rank == 0:
        wandb.init(project="galore-minipile", name=args.save_dir.split("/")[-1])
    ...
    elif args.ds == "minipile":
        data = datasets.load_dataset("JeanKaddour/minipile", split="train")
    else:
        raise NotImplementedError(f"dataset {args.ds}")
    ...
    if args.optimizer.lower() == "adam":
        optimizer = torch.optim.AdamW(trainable_params, lr=args.lr, weight_decay=args.weight_decay, betas=(0.9, 0.99))
    elif args.optimizer.lower() == "galore_adamw":
        # redefine way to call galore_adamw
        optimizer = GaLoreAdamW(param_groups, lr=args.lr, weight_decay=args.weight_decay, betas=(0.9, 0.99))
    elif args.optimizer.lower() == "io_adam":
        from io_adam.Adamw_w_grad_decompose_old_version import AdamW_Decomposed, DecomposedLinear
        optimizer = AdamW_Decomposed(trainable_params, lr=args.lr, betas=(0.9, 0.95),weight_decay=args.weight_decay, p=2, max_queue_length=args.rank, w_momentum=True)
    ...
    while Flag:
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            if update_step % args.eval_every == 0:
                ...
                logger.info(f"CUDA memory: {torch.cuda.memory_allocated()}")
            ...
            if global_rank == 0:
                ...
                mem_alloc = torch.cuda.memory_allocated("cuda")
                wandb.log({
    ...
    total_loss, evaluated_on_tokens = evaluate_model(
        model, preprocess_batched, pad_idx, global_rank, world_size, device, args.batch_size, args.ds
    )
